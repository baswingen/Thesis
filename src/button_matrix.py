...
from .stm32_reader import STM32Reader
...
class MatrixWindow(QtWidgets.QMainWindow):
    ...
    def _get_style(self, pressed: bool) -> str:
        if pressed:
    ...
    def update_state(self):
        ...
        for r in range(BUTTON_ROWS):
            for c in range(BUTTON_COLS):
                ...
                self.buttons[r][c].setStyleSheet(self._get_style(bool(is_pressed)))
...
def run_matrix_visualization(port: Optional[str] = None, baud: int = 921600):
    ...
    win = MatrixWindow(port=port, baud=baud)
    win.show()
