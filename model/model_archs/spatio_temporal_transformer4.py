...
from model.config_model import SPATIO_TEMPORAL_TRANSFORMER4_CONFIG, FEATURE_CONFIG, GLOBAL_LOSS_FUNCTION, AUGMENTATION_CONFIG
...
class SpatioTemporalTransformerNetwork4(nn.Module):
    """Modality-isolated spatio-temporal transformer.
    
    Classifies sensor channels into modality groups and isolates their spatial
    and temporal representations in the blocks, performing late fusion at the final timestep.
    """
    ...
    def __init__(self, channel_indices: dict, d_model: int, nhead_spatial: int, 
        ...
        # 4. Modality-Isolated Spatio-Temporal Blocks
        ...
        # 5. Modality-Specific Late Pooling & Concatenated Regressor
    ...
    def forward(self, x, lengths):
        ...
        # 5. Pass through Modality-Isolated Blocks
        ...
        # 7. Modality-Specific Late Attention Pooling
...
class SpatioTemporalTransformerRegressor4:
    def __init__(self, 
                 d_model: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG['d_model'],
                 nhead_spatial: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG['nhead_spatial'],
                 num_layers: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG.get('num_layers', 3),
                 nhead_temporal: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG['nhead_temporal'],
                 dim_feedforward: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG['dim_feedforward'],
                 dropout_rate: float = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG['dropout_rate'],
                 learning_rate: float = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG['learning_rate'],
                 weight_decay: float = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG.get('weight_decay', 0.0),
                 batch_size: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG['batch_size'],
                 epochs: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG['epochs'],
                 validation_split: float = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG.get('validation_split', 0.2),
                 early_stopping_patience: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG.get('early_stopping_patience', 10),
                 scheduler_patience: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG.get('scheduler_patience', 5),
                 scheduler_factor: float = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG.get('scheduler_factor', 0.5),
                 ...
                 use_checkpointing: bool = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG.get('use_checkpointing', True),
                 use_amp: bool = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG.get('use_amp', True),
                 random_state: int = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG['random_state'],
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, sample_weight=None, X_val=None, y_val=None, **kwargs):
        ...
        # Init Model
        self.model = SpatioTemporalTransformerNetwork4(
        ...
        from model.config_model import SPATIO_TEMPORAL_TRANSFORMER4_CONFIG
        scheduler_config = SPATIO_TEMPORAL_TRANSFORMER4_CONFIG.get('scheduler', {'type': 'ReduceLROnPlateau'})
        ...
        with tqdm(range(self.epochs), desc="Training Modality-Isolated ST-Transformer", unit="epoch") as pbar:
            for epoch in pbar:
                self.model.train()
                running_loss = 0.0
                n_train_samples = 0
    ...
    def load(cls, filepath: str | Path):
        ...
        regressor.model = SpatioTemporalTransformerNetwork4(
    ...
    def plot_results(self, y_test: pd.Series, y_pred: np.ndarray, save_path: str | Path):
        plotting_utils.plot_regression_results(y_test, y_pred, save_path, model_name="Modality-Isolated ST-Transformer")

    def plot_loss(self, save_path: str | Path):
        plotting_utils.plot_training_loss(self.loss_history, save_path, model_name="Modality-Isolated ST-Transformer")
