"""
Spatio-Temporal Transformer Hyperparameter Sweep
==================================================
Runs a randomised hyperparameter search for the Spatio-Temporal
Transformer with a *single* fixed cross-participant holdout split.
...
-----
    ...
    #
    # Or run standalone:
    python sweep_spatio_temporal_transformer.py
    python sweep_spatio_temporal_transformer.py --n_iter 5   # quick test
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Spatio-Temporal Transformer hyperparameter sweep"
    )
    ...
    run_dir = results_dir / f"sweep_spatio_temporal_transformer_{timestamp}"
    ...
    for i, params in enumerate(sampler, 1):
        ...
        try:
            ...
            model = SpatioTemporalTransformerRegressor(
                d_model=params['d_model'],
    ...
    print("\n" + "=" * 65)
    print("  SPATIO-TEMPORAL TRANSFORMER SWEEP COMPLETE")
    ...
    with open(report_file, "w") as f:
        f.write("SPATIO-TEMPORAL TRANSFORMER HYPERPARAMETER SWEEP REPORT\n")
        f.write("=" * 65 + "\n")
