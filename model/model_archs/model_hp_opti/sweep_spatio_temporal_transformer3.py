"""
Spatio-Temporal Transformer 3 Hyperparameter Sweep
==================================================
Runs a randomised hyperparameter search for the Modality-Grouped
Spatio-Temporal Transformer 3 with a *single* fixed cross-participant
holdout split.
...
-----
    ...
    # MODEL_TYPE = "spatio_temporal_transformer3"
    #
    # Or run standalone:
    python sweep_spatio_temporal_transformer3.py
    python sweep_spatio_temporal_transformer3.py --n_iter 5   # quick test
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Spatio-Temporal Transformer 3 (Modality-Grouped) hyperparameter sweep"
    )
    parser.add_argument("--n_iter", type=int, default=None,
                        help="Number of random configurations (default: 12 in DEV, 250 otherwise)")
    ...
    run_dir = results_dir / f"sweep_spatio_temporal_transformer3_{timestamp}"
    ...
    print("  (Modality-Grouped Temporal Attention)")
    print("=" * 65)
    ...
    print("\n" + "=" * 65)
    print("  SPATIO-TEMPORAL TRANSFORMER 3 SWEEP COMPLETE")
    ...
    for rank, res in enumerate(results[:5], 1):
        ...
        print(f"  #{rank}: RMSE={m['RMSE']:.4f} MAE={m['MAE']:.4f} R²={m['R2']:.4f} "
              f"Params={res['total_params']:,} gap={res['overfit_ratio']:.1f}x "
    ...
    with open(report_file, "w") as f:
        f.write("SPATIO-TEMPORAL TRANSFORMER 3 HYPERPARAMETER SWEEP REPORT\n")
        f.write("(Modality-Grouped Temporal Attention)\n")
        f.write("=" * 65 + "\n")
        ...
        for rank, res in enumerate(results, 1):
            ...
            f.write(f"  RMSE: {m['RMSE']:.4f} | MAE: {m['MAE']:.4f} | "
                    f"R²: {m['R2']:.4f} | Params: {res['total_params']:,}\n")
