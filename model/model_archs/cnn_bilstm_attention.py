...
from model.config_model import CNN_BILSTM_ATTENTION_CONFIG, GLOBAL_LOSS_FUNCTION, AUGMENTATION_CONFIG, GLOBAL_BALANCE_WEIGHTS
...
# ---------------------------------------------------------------------------
# CNN Feature Extractor (same multi-block backbone as CNN-LSTM / CNN-GRU)
# ---------------------------------------------------------------------------
...
class AdditiveAttention(nn.Module):
    """
    Learned additive attention that collapses a sequence of BiLSTM hidden
    states into a single fixed-length context vector.

    Input  : (batch, seq_len, hidden_dim)   — typically hidden_dim = 2 * lstm_hidden
    Output : context (batch, hidden_dim), weights (batch, seq_len, 1)
    """
    def __init__(self, hidden_dim: int, attn_units: int):
        super().__init__()
        self.score_net = nn.Sequential(
            nn.Linear(hidden_dim, attn_units),
            nn.Tanh(),
    ...
    def forward(self, lstm_out, mask=None):
        ...
        scores = self.score_net(lstm_out)          # (batch, seq_len, 1)

        if mask is not None:
            # mask: (batch, seq_len) — True for valid positions
            # Set padded positions to -inf so softmax assigns them ~0 weight
...
# ---------------------------------------------------------------------------
# Full CNN-BiLSTM-Attention Network
# ---------------------------------------------------------------------------

class CNNBiLSTMAttentionNetwork(nn.Module):
    def __init__(self, n_channels: int, cnn_filters: list[int],
                 cnn_kernel_sizes: list[int], pool_size: int,
                 lstm_hidden_size: int, lstm_num_layers: int,
                 dropout_rate: float, fc_units: int = 256):
        super().__init__()

        # --- CNN backbone (identical to CNN-LSTM) ---
        ...
        bilstm_out_dim = lstm_hidden_size * 2  # forward + backward

        # --- Attention pooling ---
        self.attention = AdditiveAttention(
            hidden_dim=bilstm_out_dim,
            attn_units=lstm_hidden_size,   # project down to half for efficiency
        )

        # --- Deep regressor head ---
        self.regressor = nn.Sequential(
            nn.Linear(bilstm_out_dim, fc_units),
            nn.ReLU(),
            nn.Dropout(dropout_rate),
    ...
    def _build_mask(self, adj_lengths, max_len):
        """Build a boolean mask (batch, max_len) where True = valid time step."""
        batch_size = adj_lengths.size(0)
        idxs = torch.arange(max_len, device=adj_lengths.device).unsqueeze(0)  # (1, max_len)
        mask = idxs < adj_lengths.unsqueeze(1)  # (batch, max_len)
        return mask

    def forward(self, x, lengths):
        # x: (batch, n_channels, time_steps)
        cnn_out = self.cnn(x)
        # cnn_out: (batch, reduced_time, cnn_filters[-1])
        ...
        # Attention with mask to ignore padded positions
        mask = self._build_mask(adj_lengths, lstm_out.size(1))
...
class CNNBiLSTMAttentionRegressor:
    def __init__(self,
                 cnn_filters: list[int] = CNN_BILSTM_ATTENTION_CONFIG['cnn_filters'],
                 cnn_kernel_sizes: list[int] = CNN_BILSTM_ATTENTION_CONFIG['cnn_kernel_sizes'],
                 pool_size: int = CNN_BILSTM_ATTENTION_CONFIG['pool_size'],
                 lstm_hidden_size: int = CNN_BILSTM_ATTENTION_CONFIG['lstm_hidden_size'],
                 lstm_num_layers: int = CNN_BILSTM_ATTENTION_CONFIG['lstm_num_layers'],
                 fc_units: int = CNN_BILSTM_ATTENTION_CONFIG.get('fc_units', 256),
                 dropout_rate: float = CNN_BILSTM_ATTENTION_CONFIG['dropout_rate'],
                 learning_rate: float = CNN_BILSTM_ATTENTION_CONFIG['learning_rate'],
                 weight_decay: float = CNN_BILSTM_ATTENTION_CONFIG.get('weight_decay', 0.0),
                 batch_size: int = CNN_BILSTM_ATTENTION_CONFIG['batch_size'],
                 epochs: int = CNN_BILSTM_ATTENTION_CONFIG['epochs'],
                 validation_split: float = CNN_BILSTM_ATTENTION_CONFIG.get('validation_split', 0.2),
                 early_stopping_patience: int = CNN_BILSTM_ATTENTION_CONFIG.get('early_stopping_patience', 30),
                 scheduler_patience: int = CNN_BILSTM_ATTENTION_CONFIG.get('scheduler_patience', 10),
                 scheduler_factor: float = CNN_BILSTM_ATTENTION_CONFIG.get('scheduler_factor', 0.5),
    ...
    def _extract_raw_segments(X: pd.DataFrame) -> list[np.ndarray]:
        ...
        if 'raw_segment' not in X.columns:
            raise ValueError(
                "CNNBiLSTMAttentionRegressor expects a DataFrame with a 'raw_segment' column. "
    ...
    def _build_network(self):
        """Instantiate the CNN-BiLSTM-Attention network."""
        return CNNBiLSTMAttentionNetwork(
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, sample_weight=None, X_val=None, y_val=None):
        ...
        if X_val is not None and y_val is not None:
            X_train, y_train = X, y
            print(f"[CNNBiLSTMAttentionRegressor] Using explicitly provided validation set ({len(X_val)} samples).")
        else:
        ...
        if AUGMENTATION_CONFIG.get('enabled', False):
            ...
            print(f"[CNNBiLSTMAttentionRegressor] Augmentation: {len(segs_train)} → "
        ...
        if self.balance_weights and self.balance_participants:
            ...
            print(f"[CNNBiLSTMAttentionRegressor] Hybrid balanced sampling enabled (Target Class × Participants). {len(combined)} training samples.")
            ...
            print(f"[CNNBiLSTMAttentionRegressor] Balanced sampling enabled (Target Class). {len(weights_np)} training samples.")
        elif self.balance_participants:
            if aug_pids is None:
                raise ValueError("balance_participants is True but no participant IDs were provided in data.")
            weights_np = compute_sample_weight(class_weight='balanced', y=aug_pids)
            sampler_weights = torch.from_numpy(weights_np.astype(np.float64))
            print(f"[CNNBiLSTMAttentionRegressor] Balanced sampling enabled (Participants). {len(weights_np)} training samples.")
        else:
        ...
        if self.loss_type == 'huber':
            ...
            print(f"[CNNBiLSTMAttentionRegressor] Using Huber (SmoothL1) loss.")
        elif self.loss_type == 'mae':
            criterion = nn.L1Loss()
        else:
            criterion = nn.MSELoss()
            if self.loss_type != 'mse':
                print(f"[CNNBiLSTMAttentionRegressor] Unknown loss '{self.loss_type}', defaulting to MSE.")

        # 6. Build datasets — no per-sample loss weights needed; sampling handles balance
        ...
        with tqdm(range(self.epochs), desc="Training CNN-BiLSTM-Attn", unit="epoch") as pbar:
            for epoch in pbar:
                # --- TRAIN ---
                self.model.train()
                running_loss = 0.0
    ...
    def load(cls, filepath: str | Path):
        ...
        # Filter out unexpectedly stored configuration keys
        ...
        regressor.model = CNNBiLSTMAttentionNetwork(
            ...
            fc_units=state['config'].get('fc_units', filtered_config.get('fc_units', 256)),
        ).to(regressor.device)

        missing_keys, unexpected_keys = regressor.model.load_state_dict(state['model_state'], strict=False)
        if unexpected_keys:
            print(f"[CNNBiLSTMAttentionRegressor] Note: ignored {len(unexpected_keys)} unexpected keys during load "
                  f"(likely normalization layer mismatch from older checkpoints).")
        if missing_keys:
            print(f"[CNNBiLSTMAttentionRegressor] WARNING: Missing keys during load: {missing_keys}")
    ...
    def plot_results(self, y_test: pd.Series, y_pred: np.ndarray, save_path: str | Path):
        plotting_utils.plot_regression_results(y_test, y_pred, save_path, model_name="CNN-BiLSTM-Attn")

    def plot_loss(self, save_path: str | Path):
        plotting_utils.plot_training_loss(self.loss_history, save_path, model_name="CNN-BiLSTM-Attn")
