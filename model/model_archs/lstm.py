...
from model.config_model import LSTM_CONFIG, FEATURE_CONFIG, GLOBAL_LOSS_FUNCTION, AUGMENTATION_CONFIG
...
class LSTMRegressor:
    def __init__(self, 
                 ...
                 weight_decay: float = LSTM_CONFIG.get('weight_decay', 0.0),
                 ...
                 scheduler_patience: int = LSTM_CONFIG.get('scheduler_patience', 5),
                 scheduler_factor: float = LSTM_CONFIG.get('scheduler_factor', 0.5),
                 scheduler: dict = None,   # absorbs per-regime scheduler injected via **LSTM_CONFIG; build reads it from config
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, sample_weight=None, X_val=None, y_val=None):
        ...
        if AUGMENTATION_CONFIG.get('enabled', True):
            ...
            print(f"[LSTMRegressor] Augmentation: {len(flat_data_train)} → {len(scaled_seqs_train)} sequences (+{n_aug} synthetic)")
        ...
        scheduler, self._sched_is_onecycle = build_scheduler(
            optimizer, LSTM_CONFIG.get('scheduler'), self.learning_rate,
        ...
        if self.loss_type == 'mae':
            ...
            if self.loss_type != 'mse':
                print(f"[LSTMRegressor] Unknown loss type '{self.loss_type}', defaulting to MSE.")
    ...
    def plot_results(self, y_test: pd.Series, y_pred: np.ndarray, save_path: str | Path):
        plotting_utils.plot_regression_results(y_test, y_pred, save_path, model_name="LSTM")

    def plot_loss(self, save_path: str | Path):
        plotting_utils.plot_training_loss(self.loss_history, save_path, model_name="LSTM")
