...
class InterleavedSTBlock(nn.Module):
    def __init__(self, d_model, nhead_spatial, nhead_temporal, dim_feedforward, dropout):
        super().__init__()
        # PyTorch MHA layers
        self.spatial_attn = nn.MultiheadAttention(d_model, nhead_spatial, dropout, batch_first=True)
        self.temporal_attn = nn.MultiheadAttention(d_model, nhead_temporal, dropout, batch_first=True)
    ...
    def forward(self, x, causal_mask=None, padding_mask=None):
        ...
        # 1. Spatial Attention (over Channels)
        ...
        # 2. Temporal Attention (over Time, per Channel)
...
class SpatioTemporalTransformerNetwork2(nn.Module):
    def __init__(self, channel_indices: dict, d_model: int, nhead_spatial: int, num_layers_spatial: int, 
        ...
        # Interleaved architecture usually stacks the block. 
        # We will use max(spatial, temporal) layers to define total block depth.
        ...
        # 3. Interleaved Spatio-Temporal Blocks
        self.blocks = nn.ModuleList([
            InterleavedSTBlock(d_model, nhead_spatial, nhead_temporal, dim_feedforward, dropout_rate)
            for _ in range(self.num_layers)
        ])
        
        # 4. Late Pooling & Regressor
        self.attention_pooling = AttentionPooling(d_model)
        self.fc = nn.Linear(d_model, 1)

    def forward(self, x, lengths):
        # x shape: [B, T, total_features]
        B, T, F = x.shape
        ...
        x_st = torch.stack(channel_tokens, dim=2)
        C = x_st.shape[2]
        ...
        # Generate dummy tensor to get valid temporal PE of shape [1, T, d_model]
        dummy = torch.zeros(1, T, self.d_model, device=x.device)
        t_pe = self.temporal_pos_encoder(dummy) # [1, T, d_model]
        x_st = x_st + t_pe.unsqueeze(2) # Broadcasting to [B, T, C, d_model]
        
        # 4. Create Masks
        causal_mask = nn.Transformer.generate_square_subsequent_mask(T, device=x.device)
        # padding_mask expects True where padded
        padding_mask = torch.arange(T, device=x.device).expand(B, T) >= lengths.unsqueeze(1)
        
        # 5. Pass through Interleaved Blocks
        ...
        last_tokens = x_st[torch.arange(B), lengths - 1, :, :] # [B, C, d_model]
        
        # 7. Late Attention Pooling over Channels
        pooled_out, _ = self.attention_pooling(last_tokens) # [B, d_model]
...
class SpatioTemporalTransformerRegressor2:
    def __init__(self, 
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, sample_weight=None, X_val=None, y_val=None, **kwargs):
        ...
        # Init Model (New Model Class)
        self.model = SpatioTemporalTransformerNetwork2(
        ...
        with tqdm(range(self.epochs), desc="Training Interleaved ST-Transformer", unit="epoch") as pbar:
            for epoch in pbar:
                self.model.train()
                running_loss = 0.0
                n_train_samples = 0
    ...
    def load(cls, filepath: str | Path):
        ...
        regressor.model = SpatioTemporalTransformerNetwork2(
    ...
    def plot_results(self, y_test: pd.Series, y_pred: np.ndarray, save_path: str | Path):
        plotting_utils.plot_regression_results(y_test, y_pred, save_path, model_name="Interleaved ST-Transformer")

    def plot_loss(self, save_path: str | Path):
        plotting_utils.plot_training_loss(self.loss_history, save_path, model_name="Interleaved ST-Transformer")
