...
class CNNLSTMNetwork(nn.Module):
    def __init__(self, n_channels: int, cnn_filters: list[int],
        ...
        self.fc = nn.Linear(lstm_hidden_size + n_static_features, 1)
...
class CNNLSTMRegressor:
    def __init__(self,
                 ...
                 scheduler: dict = None,   # absorbs per-regime scheduler injected via **CNN_LSTM_CONFIG; build reads it from config
                 ...
                 target_transform: str = CNN_LSTM_CONFIG.get('target_transform', 'none'),
                 use_anthropometrics: bool = CNN_LSTM_CONFIG.get('use_anthropometrics', False),
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, sample_weight=None, X_val=None, y_val=None):
        ...
        if self.target_transform != 'none':
            print(f"[CNNLSTMRegressor] Target transform: {self.target_transform} "
        ...
        if anthro_raw is not None:
            ...
            print(f"[CNNLSTMRegressor] Anthropometric conditioning enabled "
        ...
        scheduler, self._sched_is_onecycle = build_scheduler(
            optimizer, CNN_LSTM_CONFIG.get('scheduler'), self.learning_rate,
