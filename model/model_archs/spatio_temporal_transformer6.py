...
from model.config_model import SPATIO_TEMPORAL_TRANSFORMER6_CONFIG, FEATURE_CONFIG, GLOBAL_LOSS_FUNCTION, AUGMENTATION_CONFIG
...
# ─────────────────────────────────────────────────────────────────────────────
# Gated Multimodal Fusion Layers (SST6 Additions)
# ─────────────────────────────────────────────────────────────────────────────

class GatedSoftmaxFusion(nn.Module):
    """Coarse-Grained Gating (Option A): Computes a scalar gate weight per modality."""
    def __init__(self, d_model, num_modalities):
        super().__init__()
        self.score_projectors = nn.ModuleList([
            nn.Linear(d_model, 1) for _ in range(num_modalities)
        ])
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, modality_outs):
        # modality_outs: list of N tensors of shape [B, d_model]
        scores = torch.cat([proj(out) for proj, out in zip(self.score_projectors, modality_outs)], dim=-1) # [B, N]
        gate_weights = self.softmax(scores) # [B, N]
        
        stacked = torch.stack(modality_outs, dim=1) # [B, N, d_model]
        fused = torch.sum(stacked * gate_weights.unsqueeze(-1), dim=1) # [B, d_model]
        return fused, gate_weights

class GatedFeatureFusion(nn.Module):
    """Fine-Grained Gating (Option B - Restrained): Computes a gating vector per modality with a regularized bottleneck.
    
    Uses a compressed bottleneck projection and dropout to restrict parameter capacity
    and prevent subject-specific kinematic signature memorization in LOPO setups.
    """
    def __init__(self, d_model, num_modalities, bottleneck_dim=16, dropout_rate=0.4):
        super().__init__()
        self.num_modalities = num_modalities
        self.d_model = d_model
        
        in_features = d_model * num_modalities
    ...
    def forward(self, modality_outs):
        # modality_outs: list of N tensors of shape [B, d_model]
        concat_features = torch.cat(modality_outs, dim=-1) # [B, N * d_model]
        gates = self.gate_generator(concat_features) # [B, N * d_model]
        
        split_gates = torch.chunk(gates, self.num_modalities, dim=-1) # list of N tensors of shape [B, d_model]
        
        fused = torch.zeros_like(modality_outs[0])
        for out, gate in zip(modality_outs, split_gates):
            fused = fused + (out * gate)
            
        return fused, gates


# ─────────────────────────────────────────────────────────────────────────────
# SST6 Network with Gated Multimodal Fusion
# ─────────────────────────────────────────────────────────────────────────────

class SpatioTemporalTransformerNetwork6(nn.Module):
    """SST6: Gated Spatio-Temporal Core with Modality-Isolated Deep Layers and Late Gated Fusion.
    
    Defaults to Option B (fine-grained feature-level gating) to dynamically suppress noise
    dimensions inside non-mass-correlated modalities (e.g. IMU motion artefacts).
    """
    MODALITY_KEYWORDS = {
    ...
    def __init__(self, channel_indices: dict, d_model: int, nhead_spatial: int, 
                 ...
                 use_checkpointing: bool = False, fusion_mode: str = 'gated_feature'):
        super().__init__()
        ...
        self.num_modalities = len(self.active_groups)
        ...
        # 6. Modality-Specific Late Pooling
        self.attention_poolings = nn.ModuleDict({
            gname: AttentionPooling(d_model)
            for gname in self.active_groups
        })
        
        # 7. Adaptive Fusion & Regression Head
        if self.fusion_mode == 'gated_softmax':
            print(f"[{self.__class__.__name__}] Instantiating Coarse-Grained Gated Softmax Fusion (Option A)")
            self.fusion_layer = GatedSoftmaxFusion(d_model, self.num_modalities)
            self.fc = nn.Linear(d_model, 1)
        elif self.fusion_mode == 'gated_feature':
            print(f"[{self.__class__.__name__}] Instantiating Fine-Grained Gated Feature Fusion (Option B - Restrained)")
            self.fusion_layer = GatedFeatureFusion(d_model, self.num_modalities, bottleneck_dim=16, dropout_rate=dropout_rate)
            self.fc = nn.Linear(d_model, 1)
        else:
            print(f"[{self.__class__.__name__}] Instantiating Baseline Concatenation Fusion")
            self.fusion_layer = None
            self.fc = nn.Linear(d_model * self.num_modalities, 1)
    ...
    def forward(self, x, lengths):
        ...
        if self.fusion_layer is not None:
            fused_out, gates = self.fusion_layer(pooled_modality_outs)
...
class SpatioTemporalTransformerRegressor6:
    def __init__(self, 
                 d_model: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG['d_model'],
                 nhead_spatial: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG['nhead_spatial'],
                 num_layers: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('num_layers', 3),
                 nhead_temporal: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG['nhead_temporal'],
                 dim_feedforward: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG['dim_feedforward'],
                 dropout_rate: float = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG['dropout_rate'],
                 learning_rate: float = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG['learning_rate'],
                 weight_decay: float = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('weight_decay', 0.0),
                 batch_size: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG['batch_size'],
                 epochs: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG['epochs'],
                 validation_split: float = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('validation_split', 0.2),
                 early_stopping_patience: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('early_stopping_patience', 10),
                 scheduler_patience: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('scheduler_patience', 5),
                 scheduler_factor: float = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('scheduler_factor', 0.5),
                 ...
                 use_checkpointing: bool = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('use_checkpointing', True),
                 use_amp: bool = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('use_amp', True),
                 random_state: int = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG['random_state'],
                 max_seq_len: int = None,
                 scheduler: dict = None,
                 augmentation_config: dict = None,
                 fusion_mode: str = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('fusion_mode', 'gated_feature'),
                 **kwargs):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, sample_weight=None, X_val=None, y_val=None, **kwargs):
        ...
        self.model = SpatioTemporalTransformerNetwork6(
        ...
        from model.config_model import SPATIO_TEMPORAL_TRANSFORMER6_CONFIG
        scheduler_config = SPATIO_TEMPORAL_TRANSFORMER6_CONFIG.get('scheduler', {'type': 'ReduceLROnPlateau'})
        ...
        with tqdm(range(self.epochs), desc="Training Gated SST6 Regressor", unit="epoch") as pbar:
            for epoch in pbar:
                self.model.train()
                running_loss = 0.0
                n_train_samples = 0
    ...
    def load(cls, filepath: str | Path):
        ...
        regressor.model = SpatioTemporalTransformerNetwork6(
            ...
            fusion_mode=state['config'].get('fusion_mode', 'gated_feature')
        ).to(regressor.device)
    ...
    def plot_results(self, y_test: pd.Series, y_pred: np.ndarray, save_path: str | Path):
        plotting_utils.plot_regression_results(y_test, y_pred, save_path, model_name="Gated SST6 Regressor")

    def plot_loss(self, save_path: str | Path):
        plotting_utils.plot_training_loss(self.loss_history, save_path, model_name="Gated SST6 Regressor")
    ...
    def permutation_importance(self, X_test: pd.DataFrame, y_test: pd.Series, n_repeats: int = 5, importance_type: str = 'channel'):
        ...
        for i, fname in enumerate(self.feature_names):
            if importance_type == 'channel':
                ...
                if '_EMG_' in fname:
                    feat = fname.split('_EMG_')[-1]
                elif '_IMU_' in fname:
                    feat = fname.split('_IMU_')[-1]
                elif '_SVM_' in fname:
                    feat = fname.split('_SVM_')[-1]
                else:
                    feat = 'Anthro'
                groups[feat].append(i)
            else:
                groups[fname].append(i)
                
        importances = {}
        for group_name, idxs in tqdm(groups.items(), desc=f"Permutation importance ({importance_type})"):
            losses = []
            for _ in range(n_repeats):
                X_perm = copy.deepcopy(X_test)
                seqs = self._extract_sequences(X_perm)
                
                # Permute features for all timesteps across this channel group
                permuted_seqs = []
                for seq in seqs:
                    seq_arr = np.array([[w.get(k, 0.0) for k in self.feature_names] for w in seq])
                    if len(seq_arr) > 0:
                        # Permute the rows within the specified columns
                        shuffled_indices = np.random.permutation(len(seq_arr))
                        seq_arr[:, idxs] = seq_arr[shuffled_indices][:, idxs]
                    
                    new_seq = []
                    for row in seq_arr:
                        new_seq.append({k: row[i] for i, k in enumerate(self.feature_names)})
                    permuted_seqs.append(new_seq)
                    
                X_perm['sequence_dicts'] = permuted_seqs
                perm_preds = self.predict(X_perm)
                losses.append(mean_squared_error(y_test, perm_preds))
                
            mean_loss = np.mean(losses)
            importances[group_name] = max(0.0, mean_loss - baseline_mse)
