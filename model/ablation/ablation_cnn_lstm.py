...
def set_global_seed(seed: int):
    """Seed every RNG used by this process for full reproducibility."""
    random.seed(seed)
    np.random.seed(seed)
    try:
        import torch
...
def run_evaluation(df_subset: pd.DataFrame, loader: DataLoader,
                   target_col: str = "weight", random_state: int = 42):
    """
    Train and evaluate the CNN-LSTM on a single 80/20 stratified hold-out split.
    Returns a metrics dict with MAE, MSE, RMSE, R2.
    """
    X, y = loader.prepare_for_ml(df_subset, target_col=target_col)

    stratify = df_subset["label"] if "label" in df_subset.columns else None
...
def run_sbs(df_raw: pd.DataFrame, loader: DataLoader, target_col: str, run_dir: Path, random_state: int = 42):
    ...
    base_metrics = run_evaluation(df_raw, loader, target_col, random_state=random_state)
    ...
    if actual_emg:
        ...
        ref_metrics['EMG Only'] = run_evaluation(df_emg, loader, target_col, random_state=random_state)
    ...
    if actual_imu:
        ...
        ref_metrics['IMU Only'] = run_evaluation(df_imu, loader, target_col, random_state=random_state)
    ...
    while len(current_channels) > 1:
        ...
        for ch in current_channels:
            ...
            metrics = run_evaluation(df_slice, loader, target_col, random_state=random_state)
            rmse = metrics['RMSE']
            mae = metrics['MAE']
...
def main():
    ...
    # Fix all RNGs before anything else
    ...
    sbs_history, ref_metrics = run_sbs(df_raw, loader, target_col, run_dir, random_state=args.seed)
