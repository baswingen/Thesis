/*
  ...
  1) PRBS-15 TRIG output at 2000 Hz pin rate, 5 Hz chip rate (continuous)
  2) Dual BNO085/BNO080 UART-RVC readout (two HW UARTs)
  3) Configurable button-matrix scanning (non-blocking, debounced)

  PRBS: 5 Hz chip rate (fixed). TRIG pin updates at 2000 Hz (400 ticks per chip).
...
// --- PRBS TRIG CONFIG (chip rate fixed at 5 Hz; matches prbs_sync_testing.py) ---
#if ENABLE_PRBS_TRIG
static const uint8_t PRBS_PIN = PA8;

// 5 Hz chip rate, 2000 Hz TRIG pin output (400 ticks per chip)
static const uint32_t CHIP_RATE_HZ = 5;
static const uint32_t TRIG_OUTPUT_HZ = 2000;
static const uint32_t TICKS_PER_CHIP = TRIG_OUTPUT_HZ / CHIP_RATE_HZ;  // 400
...
// Timer ISR: 2000 Hz — each chip held for 400 ticks (continuous PRBS)
void onPrbsTick() {
  if (sub_tick == 0) {
