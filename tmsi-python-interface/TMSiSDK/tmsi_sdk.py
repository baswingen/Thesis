...
from .device.devices.legacy.legacy_device import LegacyDevice
...
class TMSiSDK(metaclass = Singleton):
    ...
    def discover(self, 
        ...
        elif dev_type == DeviceType.legacy:
            # Map dr_interface to connection type string
            conn_type_map = {
                DeviceInterfaceType.usb: 'usb',
                DeviceInterfaceType.bluetooth: 'bluetooth',
                DeviceInterfaceType.network: 'network',
                DeviceInterfaceType.wifi: 'wifi',
                DeviceInterfaceType.optical: 'fiber'
            }
            connection_type = conn_type_map.get(dr_interface, 'usb')
            TMSiLoggerActivity().log("TMSi-SDK->>LEGACY-SDK: discover devices")
            self.__legacy_device_list = LegacyDevice.discover(connection_type, num_retries)
            return (self.__legacy_device_list, [])
