...
try:
    ...
    # Let's see: does the model parameters contain the temporal_pos_encoder.pe?
    # Wait, registered buffers like pe are in state_dict, but NOT in parameters()!
    # Because register_buffer creates a non-parameter tensor.
    # Let's count parameters in the model state dict vs model parameters:
